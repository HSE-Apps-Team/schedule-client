...
const Clock = ({ fullView, setFullView }) => {
  ...
  const getPeriod = () => {
    ...
    if (schedule.length == 0) {
      ...
      schedule.forEach((period, index) => {
        if (
          currentTime >= period.startTimeUnix &&
  ...
  const lunchStatus = () => {
    ...
    if (
      userLunchPeriod.startTimeUnix <= currentTime &&
      currentTime <= userLunchPeriod.endTimeUnix
    ) {
      return "DURING";
  ...
  return status == "SCHOOL_NOW" ? (
    ...
    >
      ...
      >
        ...
        >
          {noSchoolText ? noSchoolText : statusTextData[status]["text"]}
        </h1>
        <h1
          style={{
            // color: "white",
      ...
      <img
        src={statusTextData[status]["image"]}
        className="bright"
        style={{
          margin: "5px",
          width: mobile ? "90% " : "80%",
